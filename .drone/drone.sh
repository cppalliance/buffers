...
cd __build_shared
cmake -DBOOST_ENABLE_CMAKE=1 -DBUILD_TESTING=ON -DBoost_VERBOSE=1 \
    -DBOOST_INCLUDE_LIBRARIES=$SELF -DBOOST_EXCLUDE_LIBRARIES=url -DBUILD_SHARED_LIBS=ON ..
cmake --build .
ctest --output-on-failure -R boost_$SELF
...
cmake -DBOOST_INCLUDE_LIBRARIES=$SELF -DBOOST_EXCLUDE_LIBRARIES=url -DCMAKE_INSTALL_PREFIX=~/.local ..
